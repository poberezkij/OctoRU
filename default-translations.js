// GitHub Русский Переводчик — словарь по умолчанию
// Подключается ДО content.js (см. manifest.json)

// Базовый словарь (точные соответствия UI-строк)
window.GHRU_DEFAULT_TRANSLATIONS = {
  // Авторизация / профиль
  "Sign in": "Войти",
  "Sign up": "Зарегистрироваться",
  "Sign out": "Выйти",
  "Profile": "Профиль",
  "Settings": "Настройки",

  // Профиль / меню пользователя
  "Set status": "Установить статус",
  "Your profile": "Ваш профиль",
  "Edit profile": "Редактировать профиль",
  "Your organizations": "Ваши организации",
  "Your projects": "Ваши проекты",
  "Your stars": "Ваши звёзды",
  "Your gists": "Ваши gists",
  "Copilot settings": "Настройки Copilot",
  "Feature preview": "Предпросмотр функций",
  "Appearance": "Внешний вид",
  "Accessibility": "Специальные возможности",
  "Try Enterprise": "Попробовать Enterprise",
  "Upgrade": "Улучшить тариф",

  // Разделы / сущности
  "Overview": "Обзор",
  "Organizations": "Организации",
  "Enterprises": "Предприятия",
  "Sponsors": "Спонсоры",
  "Gists": "Gists",
  "Stars": "Звёзды",
  "Packages": "Пакеты",

  // Тексты профиля
  "followers": "подписчиков",
  "following": "подписок",
  "follower": "подписчик",
  "he/him": "он",
  "Customize your pins": "Настроить закреплённые",
  "You don't have any public repositories yet.": "У вас пока нет публичных репозиториев.",
  "contributions in the last year": "вкладов за последний год",
  "Contribution settings": "Настройки вклада",
  "Learn how we count contributions": "Как мы считаем вклады",
  "Contribution activity": "Активность",
  "Show more activity": "Показать больше",
  "No activity yet for this period.": "Нет активности за этот период.",

  // Календарь вкладов
  "Less": "Меньше",
  "More": "Больше",
  "Jan": "Янв",
  "Feb": "Фев",
  "Mar": "Мар",
  "Apr": "Апр",
  "May": "Май",
  "Jun": "Июн",
  "Jul": "Июл",
  "Aug": "Авг",
  "Sep": "Сен",
  "Oct": "Окт",
  "Nov": "Ноя",
  "Dec": "Дек",
  "Mon": "Пн",
  "Tue": "Вт",
  "Wed": "Ср",
  "Thu": "Чт",
  "Fri": "Пт",
  "Sat": "Сб",
  "Sun": "Вс",

  // Футер
  "Terms": "Условия",
  "Privacy": "Конфиденциальность",
  "Status": "Статус",
  "Docs": "Документация",
  "Contact": "Контакты",
  "Manage cookies": "Настройки cookies",
  "Do not share my personal information": "Не передавать мою личную информацию",

  // Верхнее меню
  "Dashboard": "Панель",
  "Home": "Главная",
  "Feed": "Лента",
  "Pull requests": "Запросы на слияние",
  "Issues": "Задачи",
  "Marketplace": "Маркетплейс",
  "Explore": "Обзор",
  "Notifications": "Уведомления",

  // Поиск
  "Search": "Поиск",
  "Search GitHub": "Поиск по GitHub",
  "Search or jump to…": "Поиск или переход…",
  "Find a repository…": "Найти репозиторий…",
  "Find a repository...": "Найти репозиторий…",

  // Репозитории
  "Your repositories": "Ваши репозитории",
  "Repositories": "Репозитории",
  "Files": "Файлы",
  "New": "Создать",
  "Create new": "Создать",
  "Create new…": "Создать…",
  "New repository": "Новый репозиторий",
  "Create repository": "Создать репозиторий",

  // Файлы
  "Add file": "Добавить файл",
  "Upload files": "Загрузить файлы",
  "Download ZIP": "Скачать ZIP",
  "Go to file": "Перейти к файлу",
  "History": "История",

  // Вкладки репозитория
  "Code": "Код",
  "Actions": "Действия",
  "Projects": "Проекты",
  "Wiki": "Вики",
  "Security": "Безопасность",
  "Insights": "Аналитика",
  "Discussions": "Обсуждения",

  // Списки / фильтры
  "All": "Все",
  "Open": "Открытые",
  "Closed": "Закрытые",
  "Merged": "Слито",
  "Draft": "Черновик",
  "Labels": "Метки",
  "Milestones": "Вехи",
  "Assignees": "Исполнители",
  "Reviewers": "Ревьюеры",
  "Sort": "Сортировка",
  "Filter": "Фильтр",

  // PR / Review
  "New pull request": "Новый запрос на слияние",
  "Create pull request": "Создать запрос на слияние",
  "Compare & pull request": "Сравнить и создать запрос на слияние",
  "Review": "Ревью",
  "Reviews": "Ревью",
  "Approve": "Одобрить",
  "Request changes": "Запросить изменения",
  "Comment": "Комментарий",
  "Submit review": "Отправить ревью",

  // Issues
  "New issue": "Новая задача",
  "Create new issue": "Создать задачу",

  // Кнопки
  "Edit": "Редактировать",
  "Delete": "Удалить",
  "Cancel": "Отмена",
  "Save": "Сохранить",
  "Close": "Закрыть",
  "Copy": "Копировать",
  "Copied!": "Скопировано!",

  // Watch/Star/Fork
  "Watch": "Следить",
  "Unwatch": "Не следить",
  "Star": "Звезда",
  "Unstar": "Убрать звезду",
  "Fork": "Форк",
  "Existing forks": "Существующие форки",
  "Create a new fork": "Создать новый форк",
  "Fork your own copy of": "Создайте свою копию",
  "You don't have any forks of this repository.": "У этого репозитория пока нет форков.",
  "No forks yet": "Форков пока нет",
  "{N} stars": "{N} звёзд",
  "{N} watching": "{N} отслеживаний",
  "{N} forks": "{N} форков",

  // Счётчики уведомлений
  "Unread": "Непрочитанные",
  "Participating": "Участвую",
  "Starred": "Со звёздочками",

  // Прочее
  "README": "README",
  "Readme": "Прочти меня",
  "Activity": "Активность",
  "Report repository": "Пожаловаться на репозиторий",
  "No description, website, or topics provided.": "Описание, сайт и темы не указаны.",
  "No description, website, or topics provided": "Описание, сайт и темы не указаны.",
  "No releases published": "Релизы не опубликованы",
  "No packages published": "Пакеты не опубликованы",
  "[repo_home] Get started by": "Начните с",
  "[repo_home] creating a new file": "создания нового файла",
  "[repo_home] uploading an existing file": "загрузки существующего файла",
  "[repo_home] We recommend every repository include a": "Мы рекомендуем добавить в каждый репозиторий",
  "[repo_home] or": "или",
  "[repo_home] and": "и",
  "[repo_home] LICENSE": "LICENSE",
  "[repo_home] .gitignore": ".gitignore",
  "[repo_home] or uploading an existing file.": "или загрузки существующего файла.",
  "[repo_home] We recommend every repository include a README, LICENSE, and .gitignore.": "Мы рекомендуем добавить в каждый репозиторий README, LICENSE и .gitignore.",
  "Releases": "Релизы",
  "Packages": "Пакеты",
  "Contributors": "Участники",
  "Commits": "Коммиты",
  "Branches": "Ветки",
  "Tags": "Теги",
  "Compare": "Сравнить",
  "Insights": "Аналитика"
};

// Glossary mode: keep disabled to avoid noisy UI labels.
window.GHRU_GLOSSARY_TERMS = {};
